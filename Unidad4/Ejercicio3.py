...
class ArbolBinario:
    ...
    def MostrarPadreyHerm(self, subArbol, dato, anterior=None):
        if subArbol!= None:
            if dato==subArbol.getdato():
                if anterior!= None:
                    if anterior.getder()== subArbol:
                        hermano= anterior.getizq()
                    else:
                        hermano= anterior.getder()

                    print('Hermano: ', anterior.getdato())
                    if hermano!= None:
                        print('Hermano: ', anterior.getdato())
                    else:
                        print('No posee Hermano')
                else:
                    print(' El nodo del arbol no posee padre ni hermano ')
            elif dato< subArbol.getdato():
                self.MostrarPadreyHerm(subArbol.getizq(), dato, subArbol)
            else:
                self.MostrarPadreyHerm(subArbol.getder(), dato, subArbol)

    def CantidadDeNodos(self, subArbol):
        if subArbol is not None:
            return self.CantidadDeNodos(subArbol.getizq()) + self.CantidadDeNodos(subArbol.getder()) + 1

        else:
            raise Exception('Arbol vacio')

    def Sucesores(self, subArbol):
        if subArbol !=None:
            if date== subArbol.getdato():
                print('Sucesores del Nodo: ', date)
                if subArbol.HayIzquierdo():
                    self.InOrden(subArbol.getizq())
                if subArbol.HayDerecho():
                    self.InOrden(subArbol.getder())
            elif date< subArbol.getdato():
                self.Sucesores(subArbol.getizq(), date)
            else:
                self.Sucesores(subArbol.getder(), date)

        else:
            raise Exception('Nodo inexistente')
...
if __name__ == '__main__':
    ...
    print('-------ITEM A--------')
    date= int(input('Ingrese dato: '))
    ABB.MostrarPadreyHerm(raiz, date)
    print('-------ITEM B--------')
    print('Cantidad del Nodo: ', ABB.CantidadDeNodos(raiz))
    print('-------ITEM C--------')
    print('Altura del arbol: ', ABB.Altura(raiz))
    print('-------ITEM D--------')
    nodo= int(input('Ingrese nodo para mostrar sus sucesores: '))
    print('Sucesores del arbol: ', ABB.Sucesores(raiz, nodo))
